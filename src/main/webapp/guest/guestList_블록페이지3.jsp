...
<div class="container">
	...
	<div class="text-center">
		...
		<c:forEach var="i" begin="${(curBlock * blockSize)+1}" end="${(curBlock * blockSize)+blockSize}" varStatus="st">
			...
			<c:if test="${i <= totPage && pag == i}"><a href="${ctp}/GuestList?pag=${i}&pageSize=${pageSize}"><font color="red"><b>${i}</b></font></a></c:if>
			<c:if test="${i <= totPage && pag != i}"><a href="${ctp}/GuestList?pag=${i}&pageSize=${pageSize}">${i}</a></c:if>
		</c:forEach>
