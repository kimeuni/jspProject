...
//		System.out.println("pdstest에서 넘어온 파일명 : " + fName);
		...
		/*
		 	COS라이브러리에서 제공해주는 객체 : MultipartRequest() / DefaultFileRenamePolict()
		 	MultipartRequest(저장소명(request),"서버에 저장될 파일의 경로","서버에 저장될 파일의 최대용량","코드변환방식"기타옵션(파일명중복방지처리))
		 	
			ServletContext application = request.getServletContext(); // 이것에 의해서 어플리케이션 객체를 올릴 수 있음 (얘를 통해서 getRealPath() 이걸 불러올 수 있음)
			application.getRealPath("/"); // ==> 이렇게 적으면 webapp이 된다. (경로) , 이곳의 절대경로는 /web-app
		 	getRealPath() 
